...
DISPATCHER    CMPA  #START                    ; If it's the START state
              BNE   NOT_START
              JSR   START_ST                  ; then call the START_ST routine
              BRA   DISP_EXIT                 ; and exit
            
NOT_START     CMPA  #FWD                      ; Check for FORWARD state
              BNE   NOT_FWD
              JSR   FWD_ST                    ; Run FORWARD state routine
              JMP   DISP_EXIT
...
START_ST      BRCLR PORTAD0,$04,NO_FWD        ; Check if front bumper is not pressed
              JSR   INIT_FWD                  ; Initialize forward movement
              MOVB  #FWD,CRNT_STATE           ; Set the current state to FORWARD
              BRA   START_EXIT                ; Exit and return to the main loop
...
FWD_ST        BRSET PORTAD0,$04,NO_FWD_BUMP   ; Check if forward bump is detected
              JSR   INIT_REV                  ; Switch to REVERSE if bumped
              MOVB  #REV,CRNT_STATE           ; Set state to REVERSE
              JMP   FWD_EXIT                  ; Jump to FWD_EXIT and return
              
NO_FWD_BUMP   BRSET PORTAD0,$08,NO_REAR_BUMP  ; If no forward bump, check rear bump
              JSR   INIT_ALL_STP              ; Stop if rear bump is detected
              MOVB  #ALL_STP,CRNT_STATE    ; Set state to ALL_STOP
              JMP   FWD_EXIT 

NO_REAR_BUMP  LDAA  TOF_COUNTER               ; If no rear bump, load overflow counter
              CMPA  T_FWD                     ; Compare with forward time threshold
              BNE   NO_FWD_TRN                ; If less than threshold, stay in FWD
              JSR   INIT_FWD_TRN              ; If threshold exceeded, turn forward
              MOVB  #FWD_TRN, CRNT_STATE      ; Set state to FORWARD_TURN
              JMP   FWD_EXIT                  
...
REV_TRN_ST    BRSET PORTAD0,$08,NO_REAR_BUMP2 ; Check if rear bumper is pressed
              JSR   INIT_ALL_STP              ; If rear bumper is pressed, stop movement
              MOVB  #ALL_STP,CRNT_STATE       ; Set state to ALL_STOP
              JMP   FWD_EXIT                  
...
INIT_FWD      BCLR  PORTA,%00000011           ; Set FWD direction for both motors
              ...
              ADDA  #FWD_INT                  ; Add forward interval to counter
              STAA  T_FWD                     ; Store calculated time for FWD threshold
              RTS
              
;*****************************************************************

INIT_REV      BCLR  PORTA,%00000011           ; Set REV direction for both motors
              BSET  PTT,%00110000             ; Turn on the drive motors
              LDAA  TOF_COUNTER               ; Mark the rev time Tfrev
              ADDA  #REV_INT                  ; Add reverse interval to counter
              STAA  T_REV                     ; Store calculated time for REV threshold
              RTS
...
INIT_REV_TRN  BSET  PORTA,%00000010           ; Set FWD dir. for STARBOARD (right) motor
              LDAA  TOF_COUNTER               ; Mark the fwd time Tfwd
              ADDA  #REV_TRN_INT
              STAA  T_REV_TRN
              RTS
...
UPDT_DISPL    MOVB  #$90,ATDCTL5              ; r.just., unsign., sign.conv., mult., ch0, start conv.
              BRCLR ATDSTAT0,$80,*            ; wait until the conversion sequence is complete
              LDAA  ATDDR0L                   ; load the ch0 result - battery voltage - into AccA
